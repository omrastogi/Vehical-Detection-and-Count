...
def boxcheck(frame,tboxes, box):
	for tbox in tboxes:
		...
		# print ("DD-",distance,"boxe",tbox,box)
		if distance<10000:
			return False
	return True
...
def main(video):
	...
	while(1):
		...
		for box in boxes:
			if box[-1] in vehical_label: 
				if boxcheck(frame, tboxes, box):
					trackers.append(initialize_tracker(frame, box[:4].astype(np.int32)))
					cnt+=1
		# view(frame, trackers)
		...
		cv2.putText(frame, "Vehical Count : " + str(cnt), (50,600), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,10,200), 2);
